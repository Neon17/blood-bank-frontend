  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setTimeout(async () => {
      ...
      if ('message' in res && res.status === 'error') {
        ...
        alert(
          res.message +
            ': check error by seeing previous input clicking on back button consecutively'
        );
      } else {
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 flex justify-center py-10 px-2">
      <div className="w-full max-w-7xl flex flex-col items-center">
        <div className="w-full max-w-3xl bg-white dark:bg-gray-800 rounded-xl shadow-xl p-8">
          ...
          {error && (
            <div className="w-full bg-red-600 text-white dark:bg-red-400 text-black p-3 mb-5">
              <p>{error}</p>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {activeStep === 0 && (
              <div className="space-y-4">
                ...
                {errors && errors.blood_type && (
                  <div className="text-red-500">{errors.blood_type}</div>
                )}
                ...
                {errors && errors.date_time && (
                  <div className="text-red-500">{errors.date_time}</div>
                )}
                ...
                <InputField
                  ...
                  onChange={handleChange}
                />
                {errors && errors.exact_location && (
                  <div className="text-red-500">{errors.exact_location}</div>
                )}
                ...
                {errors && errors.contact_number && (
                  <div className="text-red-500">{errors.contact_number}</div>
                )}
              </div>
            )}
            ...
            {activeStep === 3 && (
              <div className="text-gray-700 dark:text-gray-300 text-sm space-y-2">
                ...
                <ul className="list-disc list-inside">
                  ...
                  <li>
                    <strong>Date:</strong> {formData.date_time}
                  </li>
                  <li>
                    <strong>Location:</strong> {formData.exact_location},{' '}
                    {location.city}
                  </li>
                  <li>
                    <strong>Contact:</strong> {formData.contact_number}
                  </li>
                </ul>
              </div>
            )}
              ...
              {activeStep < steps.length - 1 ? (
                ...
                <>
                  {submitting && (
                    <button
                      type="submit"
                      className="px-6 py-2 rounded bg-gray-800 text-white hover:bg-green-700"
                      disabled
                    >
                      Submitting...
                    </button>
