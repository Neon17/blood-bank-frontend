...
const MapPickerWrapper = dynamic(
  () =>
    import('@/app/_components/MapPickerWrapper').catch(() => {
      // Return a fallback component if import fails
      return () => (
        <div className="h-80 bg-gray-200 dark:bg-gray-700 rounded-xl flex items-center justify-center">
...
export default function DonorProfilePage() {
    ...
    const fetchData = async () => {
      try {
        ...
        if ('message' in result) {
          throw new Error(
            result.message || 'Failed to load donor profile. Please try again'
          );
        }
        ...
        setError(
          err instanceof Error ? err.message : 'Failed to load donor profile'
        );
      } finally {
        setLoading(false);
      }
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center p-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-6">
            You need to complete the donor application to create your donor
  ...
  const getStatusBadge = () => {
    ...
    if (status === verification_status.approved) {
      return (
        <span
          className={`${baseClasses} bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-300`}
        >
      ...
      return (
        <span
          className={`${baseClasses} bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-300`}
        >
      ...
      return (
        <span
          className={`${baseClasses} bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-300`}
        >
  ...
  const safeData = {
    ...
    longitude: donorData.longitude || 85.324,
          ...
          <div className="space-y-6">
            <InfoCard title="Contact Location" icon={MapPin}>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-3">
                This is your current contact location where you can be reached
                for nearby blood requests within {radius}km radius.
              </p>
            </InfoCard>
          </div>
            ...
            <InfoCard title="Medical Information" icon={Heart}>
              <div className="space-y-4">
                <div>
                  ...
                  <div className="inline-flex items-center space-x-2 bg-blue-50 dark:bg-blue-900/20 text-blue-800 dark:text-blue-300 px-3 py-1 rounded-full">
                    ...
                    <span className="capitalize font-medium">
                      {safeData.current_health_status}
                    </span>
                  </div>
                </div>
                ...
                {safeData.current_medication &&
                  safeData.current_medication !== 'None' && (
                    <div>
                      <label className="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-2">
                        Current Medications
                      </label>
